...
class ADX : public Strategy {
  ...
  void get_data() {
    std::string command = "python3 api.py ADX " + symbol + " " +
                          std::to_string(1) + " " + start + " " + end +
                          " adx.csv";
    ...
    data =
        vector<vector<double>>(close_prices_csv.size(), vector<double>(3, 0));
    for (int i = 0; i < close_prices_csv.size(); i++) {
      this->data[i] = vector<double>{stod(close_prices_csv[i]),
                                     stod(high_csv[i]), stod(low_csv[i])};
    }
  }
  ...
  Statistics get_stats(vector<vector<double>> data, vector<string> dates) {
    ...
    for (int idx = 1; idx < data.size(); idx++) {
      ...
      double DM_plus;
      double DM_minus;
      double TR = max(data[idx][1] - data[idx][2],
                      max(data[idx][1] - data[idx - 1][0],
                          data[idx][2] - data[idx - 1][0]));
      if (idx == 1) {
        ATR = TR;
      ...
      if (DX != 0) {
        if (ADX > adx_threshold) {
          if (position < x) {
            ...
            cout << dates[idx] << "->"
                 << "BUY" << endl;
            vector<string> v = {dates[idx], "BUY", "1",
                                to_string(data[idx][0])};
        ...
        if (ADX < adx_threshold) {
          if (position > -x) {
            ...
            cout << dates[idx] << "->"
                 << "SELL" << endl;
            vector<string> v = {dates[idx], "SELL", "1",
                                to_string(data[idx][0])};
            order_stats.push_back(v);
          }
        }
